...
**Battle-stat derivation -- HIGH (decoded 2026-06-14).** All five attributes (STR/SPD/VIT/INT/MND) derive from a single per-level base-stat byte -- the boot §8 level row byte `[L*9+6]` -- scaled by each job's own percent (`body[11..15]`): `attr = max(1, base_stat[level] * jobPct / 100) + equip + ability`. There is no separate per-character base attribute; the chara_set stat bytes are initial/display only. HP/MP use the same row's BE-u16 `[+2]`/`[+4]` x `body[9]`/`[10]`%. Source: `GameClass::SetJobStatus` libjniproxy.so_new.c 152644-152705 (primary), cross-checked against the FF5-PC twin `SetMemberStatus` (`FUN_00468490`, decomp 72718-72783) which uses a flat-bonus model instead (`HP = baseHP + baseHP*(VIT+jobVitBonus)/32`) -- same byte layout, different model, so libjniproxy is authoritative for FFD. base-stat grows ~20->32 over L0-15. Reproduced in `ffd/jobs/derive.py`; baked into FLVL (0.7.28); FFSmith consumes it in `host.cpp` (`memberStat`, `--jobstattest`).
