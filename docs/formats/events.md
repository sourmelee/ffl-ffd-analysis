...
- **`0x1b CameraFollow`** = `SetLookChara(entity)` — pans are camera re-targets. **`0x55 MovePlayer`** = masked 5-word position set. **`0x20 TeleportNPC`** = present-mask + indirection-mask + id/layer/x/y/dir. **`0x21`** = chara visible flag 0x400. **`0x2a SetFade`** = mode, type, RGBA, BE-word duration, block-until-done flag.
